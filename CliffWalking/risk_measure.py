...
class RiskMeasure():
            ...
            for idx1, row in enumerate(x):
                for idx2, col in enumerate(row.transpose(0,1)):
                    ...
                    sol = optimize.root_scalar(f,
                                            bracket=[np.quantile(col_np,1-self.alpha)-10, np.quantile(col_np,1-self.alpha)+10])
                    lambda0 = sol.root
                    weights[idx1,:,idx2] = T.minimum(T.exp((col.detach()-lambda0-self.kappa)/self.kappa), T.tensor(1/self.alpha))
            
            RM = T.mean(weights*x, axis=1) - self.kappa*T.mean(weights*T.log(weights+1e-7), axis=1)
    ...
    def compute_risk_np(self, x):
        ...
        # Conditional value-at-risk penalized
            ...
            sol = optimize.root_scalar(f, bracket=[np.quantile(x,1-self.alpha)-10, np.quantile(x,1-self.alpha)+10])
            lambda0 = sol.root
            weights = np.minimum(np.exp((x-lambda0-self.kappa)/self.kappa), 1/self.alpha)
            RM = np.mean(weights*x) - self.kappa*np.mean(weights*np.log(weights+1e-7))
        ...
        # Conditional value-at-risk penalized
            ...
            for idx1, row in enumerate(V_tp1):
                for idx2, col in enumerate(row.transpose(0,1)):
                    ...
                    sol = optimize.root_scalar(f, bracket=[np.quantile(col_np,1-self.alpha)-10, np.quantile(col_np,1-self.alpha)+10])
            ...
            loss = T.mean(weights * logprob * (V_tp1 - self.kappa*T.log(weights+1e-7) - lambda0.unsqueeze(1)), axis=1)
