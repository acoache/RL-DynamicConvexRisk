...
class RiskMeasure():
    ...
    def compute_risk_np(self, x):
        ...
        # Conditional value-at-risk penalized
            ...
            sol = optimize.root_scalar(f, bracket=[np.quantile(x,1-self.alpha)-20, np.quantile(x,1-self.alpha)+20])
            lambda0 = sol.root
        ...
        # Conditional value-at-risk penalized
            ...
            for idx1, row in enumerate(V_tp1):
                for idx2, col in enumerate(row.transpose(0,1)):
                    ...
                    sol = optimize.root_scalar(f, bracket=[np.quantile(col_np,1-self.alpha)-20, np.quantile(col_np,1-self.alpha)+20])
