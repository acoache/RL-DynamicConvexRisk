...
simulations = np.zeros((Nsimulations, params["T"], len(rm_list))) # matrix to store all testing trajectories
...
for idx_method, method in enumerate(rm_list):
    ...
    # ensemble of neural networks; Ndt-1 policies and value functions
    ...
    for t_t in env.spaces["t_space"][:-1]:
        ...
        simulations[:,t_t,idx_method] = r.detach().numpy()

    simulations[:,-1,idx_method] = env.get_final_reward(pos).detach().numpy()
